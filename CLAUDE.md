...
| Agent | `agent/tools.py`, `agent/planner.py`, `agent/solver.py` | Done |
| Metrics | `metrics/store.py` | Done |
| API routes | `api/routes/chat.py`, `api/routes/metrics.py` | Done |

All steps complete. `POST /chat` is live; `GET /metrics`, `GET /metrics/history`, and `POST /evaluate` are implemented.
